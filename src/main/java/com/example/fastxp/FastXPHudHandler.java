...
public class FastXPHudHandler {
    ...
    public static void onRenderHud(CustomizeGuiOverlayEvent.DebugText event) {
        ...
        if (health <= 6.0f && !mc.player.getMainHandItem().is(Items.TOTEM_OF_UNDYING) && !mc.player.getOffhandItem().is(Items.TOTEM_OF_UNDYING)) {
            ...
            int textY = height - 68; 
            int alpha = (int) (Math.sin((float)mc.player.tickCount * 0.4f) * 85 + 170);
            int blinkColor = (alpha << 24) | 0xFF2222;
            guiGraphics.drawString(mc.font, warningText, textX, textY, blinkColor, true);
        }

        // =========================================================================
        // ОТРЕСОВКА БРОНИ И ЕЕ ПРОЧНОСТИ (Справа от инвентаря!)
        // =========================================================================
        ...
        // =========================================================================
        // СЧЕТЧИК ТОТЕМОВ И ЯБЛОК В ПЛАШКАХ (Сдвинут ещё правее брони)
        // =========================================================================
        int totemCount = 0;
        int gappleCount = 0;
